...
load("../Data/woaData_USEA_components2.rdata")
...
XR <- XG <- X[,c(1:2, 4:8,10:11,14:17)] 
ZG <- cbind(1, X[, c("xconst","parcomp","press.use", "rgdpna", "Incomp" ,"lmtnest", "ethfrac", "relfrac")])
llik <- function(x0){
  lik(x0, XR=XR, XG=XG, Z=ZG, y=y, rwin=rwin)
}

upper <- rep(20, ncol(XR)*2+2*ncol(ZG))
lower <- -upper
set.seed(1)
...
deTest <- DEoptim(llik, lower, upper,
                  control=DEoptim.control(parallelType=1,
                                          ...
                                          steptol=100000))
outerit <- outerit+ 1
save(list="deTest", file="model9start_currentiteration.rdata") 
...
# save.image("model9_startvalues.rdata")  #only if regenerating starting values
