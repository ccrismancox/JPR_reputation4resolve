...
load("../Data/woaData_USEA_intensity.rdata")
...
ZG <- cbind(1, X[, c("polity2", "rgdpna" ,"Incomp", "lmtnest", "ethfrac", "relfrac", "IntensityLevel")])
llik <- function(x0){
...
deTest <- DEoptim(llik, lower, upper,
                  control=DEoptim.control(parallelType=1,
                                          ...
                                          steptol=100000))
outerit <- outerit+ 1
save(list="deTest", file="model8start_currentiteration.rdata") 
...
# save.image("model8_startvalues.rdata")  #only if regenerating starting values
