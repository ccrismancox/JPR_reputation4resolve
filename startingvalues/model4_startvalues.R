...
ZG <- XR[,-c(9:10)] #coup and oil lead to model explosion
llik <- function(x0){
...
deTest <- DEoptim(llik, lower, upper,
                  control=DEoptim.control(parallelType=1,
                                          ...
                                          steptol=100000))
outerit <- outerit+ 1
save(list="deTest", file="model4start_currentiteration.rdata") 
...
# save.image("model4_startvalues.rdata")  #only if regenerating starting values
